package com.storyplayer.phonemidea;
...
import com.storyplayer.dbhandler.FavoritePlayTableHelper;
import com.storyplayer.dbhandler.MostAndRecentPlayTableHelper;
...
public class PhoneMediaControl {
    ...
    public Uri addMusicItem(final Context context, final String songname) {
    String path = new String(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MUSIC).getAbsolutePath()+ File.separator+"Dadi"+ File.separator+songname+".mp3");
...
//        TITLE,DATE_ADDED,DATE_MODIFIED,MIME_TYPE
    ...
    public ArrayList<SongDetail> getList(final Context context, final long id, final SonLoadFor sonloadfor, final String path) {
        ...
        switch (sonloadfor) {
            case All:
                String mediapath = new String(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MUSIC).getAbsolutePath()+ File.separator+"Dadi"+ File.separator+"%");
                String selection = MediaStore.Audio.Media.IS_MUSIC + " != 0";
//                String selection = MediaStore.Audio.Media.DATA + " != 0 AND " +
//                MediaStore.Audio.Media.DATA + " LIKE '"+ mediapath +"'";
//              sortOrder = MediaStore.Audio.Media.DATE_ADDED + " desc";
                sortOrder = MediaStore.Audio.Media.DEFAULT_SORT_ORDER;
                cursor = ((Activity) context).getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, projectionSongs, MediaStore.Audio.Media.DATA + " like ? ", new String[] {"%/Dadi/%"}, sortOrder);
    ...
    public static void runOnUIThread(Runnable runnable, long delay) {
        if (delay == 0) {
            ApplicationStoryPlayer.applicationHandler.post(runnable);
        } else {
            ApplicationStoryPlayer.applicationHandler.postDelayed(runnable, delay);
        }
    }
