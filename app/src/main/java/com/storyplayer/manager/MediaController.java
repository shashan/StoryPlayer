package com.storyplayer.manager;
...
public class MediaController implements NotificationManager.NotificationCenterDelegate, SensorEventListener {
    ...
    public boolean playAudio(SongDetail mSongDetail) {
        ...
        if (MusicPreferance.playingSongDetail != null) {
            Intent intent = new Intent(ApplicationStoryPlayer.applicationContext, MusicPlayerService.class);
            ApplicationStoryPlayer.applicationContext.startService(intent);
        } else {
            Intent intent = new Intent(ApplicationStoryPlayer.applicationContext, MusicPlayerService.class);
            ApplicationStoryPlayer.applicationContext.stopService(intent);
        }

        storeResendPlay(ApplicationStoryPlayer.applicationContext, mSongDetail);
    ...
    private void startProgressTimer() {
        synchronized (progressTimerSync) {
            ...
            progressTimer.schedule(new TimerTask() {
                ...
                public void run() {
                    synchronized (sync) {
                        StoryPlayerUtility.runOnUIThread(new Runnable() {
                            @Override
                            public void run() {
     ...
     * Store Rcent Play Data
     */
    public synchronized void storeDownloading(final Context context, final Image mDetail) {

        AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
            @Override
            protected Void doInBackground(Void... voids) {
                try {
                    DownloadingTableHelper.getInstance(context).inserDownloadingSong(mDetail);
//                    MostAndRecentPlayTableHelper.getInstance(context).inserSong(mDetail);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public static boolean isMyServiceRunning(Context context) {
        ...
        for (RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
            if (SongDownloadService.class.getName().equals(service.service.getClassName())) {
                return true;
            }
        }
        return false;
